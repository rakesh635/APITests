Feature: Testing all the product rest APIs
  ...
  Scenario: Get the full product list
		Given url 'http://localhost:8080/product/list'
		When method GET
		Then status 200
  ...
  Scenario: Add a new product to the list
		Given url 'http://localhost:8080/product/add'
  ...
  Scenario: Delete the 3rd product from the product list
		Given url 'http://localhost:8080/product/delete/3'
		When method DELETE
		Then status 200
  ...
  Scenario: Update the price of the first product in the list
		Given url 'http://localhost:8080/product/update/1'
